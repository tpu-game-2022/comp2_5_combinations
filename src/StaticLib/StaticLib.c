...
void comb(int tmp_sum, int set_count, int sum, const int* val_array, int array_size, int* result)
{
	if (set_count == array_size)
	{
		if (tmp_sum == sum) (*result)++;
	...
	comb(tmp_sum + val_array[set_count], set_count + 1, sum, val_array, array_size, result);

	comb(tmp_sum, set_count + 1, sum, val_array, array_size, result);
}
...
int calc_combinations(int sum, const int* val_array, int array_size)
{
	...
	comb(0, 0, sum, val_array, array_size, &result);
